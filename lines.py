...
def cmyk_crosshatch(img, layers = 10, number = 30, angle = 35):
    R = img[:, :, 2]
    G = img[:, :, 1]
    B = img[:, :, 0]
    C = np.zeros(R.shape)
    ...
    clines = crosshatch(255 - C, layers = layers, number = number, angle = angle)
    mlines = crosshatch(255 - M, layers = layers, number = number, angle = angle + 45)
    ylines = crosshatch(255 - Y, layers = layers, number = number, angle = angle + 90)
    klines = crosshatch(255 - K, layers = layers, number = number, angle = angle + 135)
...
if __name__ == "__main__":
    # img = np.array(cv2.imread('mark.jpg'))
    ...
    # clines, mlines, ylines, klines = cmyk_crosshatch(img, layers = 8, number = 30)
