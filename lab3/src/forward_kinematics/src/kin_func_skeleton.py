...
def rotation_3d(omega, theta):
    ...
    return np.identity(3) + np.sin(omega_norm * theta) / omega_norm * omega_hat \
        + (1 - np.cos(omega_norm * theta)) / (omega_norm * omega_norm) * (omega_hat.dot(omega_hat))
...
def homog_3d(xi, theta):
    ...
    if w_norm == 0:
        ...
        g[:3,:3] = rotation_3d(w, theta)
        g[:3,3] = ((np.identity(3) - g[:3,:3]).dot(xi_hat[:3,:3].dot(v)) + np.outer(w, w).dot(v) * theta) / (w_norm * w_norm)
    return g
