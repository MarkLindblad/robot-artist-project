...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    return ((gray_img >= lower_thresh) & (gray_img <= upper_thresh)).astype(int)
...
def edge_detect_naive(gray_img):
    ...
    # blur = 1/273 * np.array([[1, 4, 7, 4, 1], [4, 16, 26, 16, 4], [7, 26, 41, 26, 7], [4, 16, 26, 16, 4], [1, 4, 7, 4, 1]])
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    cluster_img = np.reshape(clusters, img_d.shape[:2])
...
def segment_image(img): 
    ...
    binary = threshold_segment_naive(to_grayscale(img), 60, 80).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    filename = "legos.jpg" if len(sys.argv) == 1 else sys.argv[1]
    test_img = read_image(IMG_DIR + '/' + filename, grayscale=True)
    test_img_color = read_image(IMG_DIR + '/' + filename)

    # uncomment the test you want to run
    # it will plot the image and also save it

    test_thresh_naive(test_img, 50, 200)
