...
def gray_dither(img, size = 50, black = 0, white = 255, scatter = 0, off_x = 0, off_y = 0, angle = 0):
    ...
    unique, number = np.unique(bits, return_counts = True)

    return dots_x, dots_y, number[np.where(unique == 0)]

def color_dither(img, size = 50, black = 0, white = 255, scatter = 0, thickness = 0.5):
    R = img[:, :, 2]
    G = img[:, :, 1]
    B = img[:, :, 0]
    C = np.zeros(R.shape)
    ...
    c_x, c_y, c_number = gray_dither(255 - C, black = black, white = white, angle = 0, size = size)
    m_x, m_y, m_number = gray_dither(255 - M, black = black, white = white, angle = 30, size = size)
    y_x, y_y, y_number = gray_dither(255 - Y, black = black, white = white, angle = 45, size = size)
    k_x, k_y, k_number = gray_dither(255 - K, black = black, white = white, angle = 60, size = size)

    print(f'Number of Cyan dots:    {c_number}')
    print(f'Number of Magenta dots: {m_number}')
    print(f'Number of Yellow dots:  {y_number}')
    print(f'Number of Black dots:   {k_number}')
    print(f'Number of Total dots:   {c_number + m_number + y_number + k_number}')

    return (c_x, c_y, m_x, m_y, y_x, y_y, k_x, k_y, size, thickness)

def color_dither_display(arr):
    c_x, c_y, m_x, m_y, y_x, y_y, k_x, k_y, size, thickness = arr
...
color_dither_display(color_dither(img, thickness = 0.7, size = 50))

# img = np.array(cv2.imread('me.png', 0))
# dots_x, dots_y, dots = gray_dither(img, angle = 30, size = 200)
